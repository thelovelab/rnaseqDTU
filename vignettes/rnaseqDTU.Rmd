...
pre-constructed matrix of counts. In order to comply with Bioconductor 
workflow data size requirements, we also trimmed the counts matrix to
two decimal places per count (which is more than enough precision on
estimated counts). The original analysis was run on the counts
directly from the *Salmon* quantification files. These quantification
files for this dataset have been made publicly available; see the
*Data availability* section at the end of this workflow.
...
that also takes as input *Salmon* quantification, and so, for
...
**Note:** in the following evaluations, `nbinomLRT` was incorrectly
used in the *DEXSeq* workflow instead of the correct `testForDEU`. The
evaluation plots will therefore need to be replaced with updated
versions. The `dxr` and `dxr.g` tables have already been updated
(August 8, 2018).

We begin the evaluation by noting that all of the methods correctly avoided
calling many of the DGE events as DTU events. The object `dge.genes` contains the names
...
in contrast to the OFDR where the unit of false discovery is the *gene*.
...
```{r ofdr-pgd, out.width="75%", echo=FALSE, fig.cap="Number of true positives and observed overall false discovery rate (OFDR) using stageR for 5\\% target, on the simulation with fixed per-gene dispersions."}
knitr::include_graphics("figs/ofdr_pergene_disp.jpg")
```
...
described in the earlier sections of the workflow represents an easily
...
The work of MIL on this workflow was supported by R01 HG009125, P01 CA142538,
and P30 ES010126. CS declared that no grants were involved in supporting this
work. The work of RP on this workflow was supported by NSF BIO-1564917, and
CCF-1750472.
