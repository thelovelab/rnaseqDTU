...
gene expression) and "DTE" (differential transcript expression) 
but did not count as "DTU" (differential transcript
...
genes counted for DTU and DTE, but not for DGE. An MA plot of the simulated
...
sample CSV file which records which samples are condition 1 and which are
...
its FDR bounds, particularly on the transcript-level tests, and 
...
For size consideration, we reduce also the transcript-level results table to a simple *data.frame*:
