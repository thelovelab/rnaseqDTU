...
TODO fix `getDiffProp`

```{r}
# this is a quick hack. before submitting, i will
# re-write it to be safe to its assumptions and have stop()'s
getDiffProp <- function(d, n.sub) {
...
res.txp.filt$dprop <- getDiffProp(d, 6)
...
methods (including post-hoc filtering option for *DRIMSeq*). Here we
simply looked across the transcript-level adjusted p-values without
regard for which transcripts came from which gene. *SUPPA2* again
tended to controlled its FDR, as did *DEXSeq*. *DRIMSeq* with
difference-in-proportion filtering approached the target FDR sample
