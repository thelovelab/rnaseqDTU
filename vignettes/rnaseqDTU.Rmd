...
abstract: |
  ...
  gene expression analysis **NOTE: Do we want to mention DTE in the abstract?**. Here we present a simple workflow using a
...
usage was constructed by exchanging the abundance **NOTE: In terms of count, or TPM?** of two expressed
...
To build the *Salmon* index, we used the following command **NOTE: Maybe note here that this will discard sequence duplicate transcripts**:
...
should be considered with a method's sensitivity in mind as well.
...
target **NOTE: Maybe mention the sensitivity here?**. *DRIMSeq* and *DEXSeq* were the most sensitive methods in
