...
samples in total gene expression -- the total number of transcripts
produced by all isoforms of a gene -- and additionally differences in
transcript usage within a gene. If the number of expressed transcripts [Charlotte: I think this can be misinterpreted as the number of isoforms that are expressed]
...
*DRIMSeq* publication is a good reference and has descriptions and
...
followed by *confirmation* of which transcripts show differential usage
with transcript-level adjusted p-values (cite). The method provides
...
experimental parameters estimated from the GEUVADIS dataset, so that [Charlotte: it sounds a bit like using the GEUVADIS dataset to estimate parameters is a prerequisite for being able to evaluate different types of differential behaviour]
...
link: <https://github.com/mikelove/swimdown>. *alpine* was used to
...
Binomial distribution for gene-level [Charlotte: I guess one possible question here is whether gene-level mean-dispersion estimates are valid also on the transcript level - maybe worth a comment?] counts for 462 GEUVADIS samples
...
sample of the GEUVADIS project (cite Salmon) [Charlotte: I felt that this sentence should maybe be close to the one discussing mean-dispersion estimates above]. *countsimQC* was used to
...
abundances for the two groups is shown in Figure \@ref(fig:ma-simulated).[Charlotte: I can't actually see the grey points in the plot]

```{r ma-simulated, message=FALSE, echo=FALSE, dev="png", out.width="50%", fig.cap="MA plot of simulated abundances. Each point depicts a transcript, with the average of log2 abundance (TPM) on the x-axis and the difference between the two groups on the y-axis. Of the transcripts which were expressed with TPM > 1 in at least one group, 77\\% are null transcripts (grey), and 23\\% are differentially expressed (green, orange, or purple)."}
load("simulate.rda")
library(rafalib)
bigpar()
pc <- 1
...
later step [Charlotte: CSV and quant files will be part of the workflow package in the end, or should users download/create them?].
...
TODO more details about the filter
...
target. We have written a simple function for assessing the difference
in fitted proportions between two groups, but it is not general for
all designs. [Charlotte: this is not really "independent" filtering, and so I wonder if it's worth mentioning this - we couldn't necessarily expect (theoretically) that the FDR should be controlled at the same level as before anymore]

```{r}
getDiffProp <- function(d, design_full) {
  # test first to make sure the design is a simple two group
  stopifnot(ncol(design_full == 2) & all(design_full[,1] == 1) & all(design_full[,2] %in% 0:1))
  # identify a sample from each group
  samp.idx <- sapply(0:1, function(i) which(design_full[,2] == i)[1])
  # pull out the fitted proportions
  fitted <- proportions(d)[,2 + samp.idx]
  # absolute difference
  abs(fitted[,2] - fitted[,1])
}
res.txp.filt <- DRIMSeq::results(d, level="feature")
res.txp.filt$dprop <- getDiffProp(d, design_full)
res.txp.filt$pvalue[res.txp.filt$dprop < .1] <- 1
res.txp.filt$adj_pvalue[res.txp.filt$dprop < .1] <- 1
```
...
per-gene adjusted p-value, using the *perGeneQValue* function [Charlotte: worth mentioning other ways, e.g. discussed in https://genomebiology.biomedcentral.com/articles/10.1186/s13059-018-1419-z], which
...
transcripts with TPM larger than 1 [Charlotte: is there a reason that the DRIMSeq filtering was not used from the beginning here?], and did not use gene-correction,
...
851 with an adjusted p-value less than 0.05. [Charlotte: maybe add a comment in the code to highlight which method corresponds to each table]
...
knitr::include_graphics("figs/dtu_gene.jpeg")
```
...
[Charlotte: all other plots have white background - maybe this one should too :)]
```{r dtu-ofdr, out.width="75%", echo=FALSE, fig.cap="Number of true positives and observed OFDR using stageR for 5\\% target. Each method is drawn as a line, and the numbers to the right of the points indicate the per-group sample size. Adjusted p-values for a nominal 5\\% OFDR (dashed vertical line) were generated for DEXSeq and DRIMSeq (with and without post-hoc filtering) from gene- and transcript-level p-values using the stageR framework for stage-wise testing."}
...
knitr::include_graphics("figs/dtu_txp.jpeg")
```
...
: (\#tab:timing-dtu) Timing of methods for DTU in minutes by per-group sample size.

TODO: check why 18 minutes for DRIMSeq three times?

| Method | n=3 | n=6 | n=9 | n=12 |
| --- | --- | --- | --- | --- | 
| *DRIMSeq* | 0:15 | 0:18 | 0:18 | 0:18 |
...
: (\#tab:timing-dge) Timing of methods for DGE in minutes by per-group sample size.
...
: (\#tab:timing-dte) Timing of methods for DTE in minutes by per-group sample size.
