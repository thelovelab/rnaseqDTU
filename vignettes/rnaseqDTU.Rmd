...
abundances for the two groups is shown in Figure \@ref(fig:ma-simulated).
...
At the gene-level, in terms of controlling at
...
tended to controlled its FDR, as did *DEXSeq* (Figure \@ref(fig:dtu-txp)). 
...
| --- | --- | --- | --- | --- | 
| *DRIMSeq* | 0:30 | 0:18 | 0:18 | 0:18 |
| *DEXSeq* | 0:01 | 0:02 | 0:04 | 0:07 |
| *SUPPA2* | 0:16 | 1:18 | 3:48 | 5:33 |

# Differential gene expression

TODO: add text here to explain what you're doing
...
TODO: explain that the pairing of methods in the next two plots is
fairly arbitrary
...
jitter.padj <- -log10(dxr.g$qval + 1e-16)
jp.idx <- jitter.padj == 16
jitter.padj[jp.idx] <- rnorm(sum(jp.idx),16,.25)
...
1. All software used in this workflow should be installed from the release branch of Bioconductor.
