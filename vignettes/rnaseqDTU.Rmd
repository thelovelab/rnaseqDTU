...
abstract: |
  ...
  pipeline is fast, benefiting from previous inference steps by Salmon
...
[@Glaus2012Identifying;@Anders2012Detecting;@Robinson2009EdgeR;@McCarthy2012Differential;@Smyth2004Linear;@Law2014Voom;@Nowicka2016DRIMSeq;@Van2017StageR;@Vitting2017]. 
...
infrastructure, please refer to the [@Huber2015Orchestrating].
...
an estimator of fold change based on the gene-level counts [@Trapnell2013Differential].
...
paired-end reads can be found at an associated GitHub repository for
the simulation code [@swimdown].
...
project from the same center. *DESeq2* [@Love2014Moderated] was
...
transcripts of a gene had counts generated using the same per-gene
dispersion.
...
more. We used all three possible filters, whereas only the two count
...
differential exon usage [@Anders2012DEXSeq], but can also be adapted to run on estimated
transcript counts, in order to detect DTU. This
...
valuable to investigators than per-transcript results, depending on the system
...
proportion SD filtering lowered to less than 10% as the sample
...
using *iCOBRA*.  This analysis looked across the transcript-level
adjusted p-values for DTU without using the information associating
transcripts to gene. *SUPPA2* again tended to control its FDR, as did
...
the observed FDR was always higher than the target.  While we
performed this last evaluation on the independently adjusted p-values
from the transcript-level, we recommend instead to use the stage-wise
framework and adjusted p-values provided by the *stageR* method,
assessed in Figure \@ref(fig:dtu-ofdr).
...
genes were assigned dispersion parameters by matching the gene-level
...
transcripts of a gene being assigned the same dispersion parameter. This
contrasts with the main simulation, in which each transcript was
assigned its own dispersion parameter, resulted in heterogeneity of
dispersion within a gene. As we do not know the degree to which
transcripts of a gene would have correlated biological variability, we
...
*DRIMSeq* nearly controlled FDR at the gene-level in the
...
```{r dtu-gene-pgd, out.width="75%", echo=FALSE, fig.cap="Gene-level screening for DTU, on the simulation with fixed per-gene dispersions. The four panels shown are for per-group sample sizes: (A) 3, (B) 6, (C) 9, (D) 12. Circles indicate thresholds of 1\\%, 5\\% and 10\\% nominal FDR."}
...
```{r dtu-txp-pgd, out.width="75%", echo=FALSE, fig.cap="Transcript-level DTU analysis without stage-wise testing, on the simulation with fixed per-gene dispersions. The four panels shown are for per-group sample sizes: (A) 3, (B) 6, (C) 9, (D) 12. Circles indicate thresholds of 1\\%, 5\\% and 10\\% nominal FDR."}
...
We compared *DESeq2*, *EBSeq*, *edgeR*, *edgeR-QL* (using the
quasi-likelihood functions), *limma* with *voom* transformation,
*SAMseq*, and *sleuth*. We used *tximport* to summarize counts to the
gene level, and provided all methods other than *DESeq2* and *sleuth*
with the `lengthScaledTPM` counts matrix. *sleuth* takes as input the
quantification from *kallisto*, which was run with 30 bootstrap
...
counts and an offset from *tximport*, we used this approach. *edgeR*
...
link: <https://github.com/mikelove/swimdown>. Timings for the
...
whether from the simulated "DGE", "DTE", or "DTU" genes. For most of
...
showed how the transcript usage results can complement a gene-level
...
used the simulation dataset to evaluate Bioconductor and other R
...
One limitation of the workflow presented here is that, in contrast to
other methods such as the standard *DEXSeq* analysis or *SUPPA2*, here
we consider and detect expression switching between annotated
transcript. Other methods such as *DEXSeq* and *SUPPA2* may benefit in
terms of power and interpretability from performing statistical
analysis directly on exon usage or splice events, whereas the pipeline
presented here would require further processing to attribute
transcript usage changes to specific splice events.
